...
class PageTest(TestCase):
    ...
    def create_category_with_article(self):
        article = self.create_article()
        category = article.categories.create(
            slug="category_slug",
            name="category_name",
        )
    ...
    def test_getting_category_page(self):
        category, article = self.create_category_with_article()
        response = self.client.get(reverse('category_page', args=(category.slug, )))

        self.assertEqual(response.context['category_slug'], category.slug)
        self.assertIn(article, response.context['page'].object_list)
    ...
    def test_getting_category_page_with_page_num(self):
        category, article = self.create_category_with_article()
        response = self.client.get(reverse('category_page_with_page_num', args=(category.slug, 1, )))
