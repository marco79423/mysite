...
@task
def setup_frontend_serv(project_path):
    with cd(project_path):
        sudo('docker-compose pull')

@task
def restart_frontend_serv(project_path):
    with cd(project_path):
...
def deploy(target, branch, prod):
    ...
    if target in ('frontend', 'all'):
        execute(setup_frontend_serv, project_path)
        execute(restart_frontend_serv, project_path)
